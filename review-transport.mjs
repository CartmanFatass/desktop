...
import {
  CHATGPT_REVIEW_PRODUCT_MODELS,
  CHATGPT_REVIEW_REASONING_EFFORT,
...
export async function archiveReviewResponse({ responsePath, text }) {
  ...
  try {
    ...
    try {
      ...
      // Windows cannot fsync a directory handle (EPERM/EISDIR/EINVAL); the linked
      // file itself was already synced above, so the archive bytes are durable.
...
function normalizeRequest(input) {
  ...
  if (provider === 'chatgpt' && !CHATGPT_REVIEW_PRODUCT_MODELS.includes(productModel)) fail('review_invalid_request', { field: 'productModel' });
  if (provider === 'chatgpt' && reasoningEffort !== CHATGPT_REVIEW_REASONING_EFFORT) fail('review_invalid_request', { field: 'reasoningEffort' });
...
export async function runReviewQuery({ stateDir, tabs, request: rawRequest, onTabResolved = null }) {
  ...
  const intake = await mutateState(stateDir, async (state) => {
    ...
    // A replacement key is not a recovery mechanism for the same submitted
    // question. Include the observed identity of a first-bound conversation.
    for (const operation of Object.values(state.operations)) {
      const identity = operationConversation(operation);
      if (operation.sendAttempted && operation.provider === request.provider &&
          operation.promptSha256 === request.promptSha256 &&
          identity.conversationUrl === request.conversationUrl &&
          identity.conversationId === request.conversationId) {
        fail('review_duplicate_submission');
      }
    }
  ...
  const onPrepared = async (prepared) => {
    ...
    await mutateState(stateDir, async (state) => {
      ...
      operation.baselineMessageIds = baselineMessageIds;
      operation.updatedAt = Date.now();
    });
  };
  ...
  const onSendAttempted = async () => {
    await mutateState(stateDir, async (state) => {
      ...
      // before either reached the send boundary.
      for (const other of Object.values(state.operations)) {
        const identity = operationConversation(other);
        if (other.operationId !== operation.operationId && other.sendAttempted &&
            other.provider === request.provider && other.promptSha256 === request.promptSha256 &&
            identity.conversationUrl === request.conversationUrl &&
            identity.conversationId === request.conversationId) fail('review_duplicate_submission');
      }
  ...
  const onUserTurnObserved = async (observed) => {
    ...
    await mutateState(stateDir, async (state) => {
      ...
      if (
        ...
        operation.operationId !== intake.operation.operationId ||
        !operation.sendAttempted ||
        operation.baselineMessageIds?.includes(providerUserMessageId) ||
  ...
  try {
    ...
    const execution = await runExclusive(async () => {
      ...
      if (!current.providerUserMessageId) {
        // Older v4 receipts can still be read, but cannot be guessed into a
        // submitted identity when their pre-send baseline was never persisted.
        if (!Array.isArray(current.baselineMessageIds)) fail('review_submission_baseline_unavailable');
